...
class TestPieces(unittest.TestCase):
    ...
    def test_count_occupied_positions(self):
        board = Board(3,3)
        board.add_piece('Rook')
        board.add_piece('Rook')
        rook_1 = board.pieces[0]
        rook_2 = board.pieces[1]
        board.set_piece(rook_1, Position(0,0))
        board.set_piece(rook_2, Position(2,2))
    ...
    def test_piece_positioning(self):
        ...
        #Test if i put the Rook on (0,1) will harm the Bishop, and it will
        self.assertEqual(1,board.will_harm(rook,Position(0,1)))

        rook_position = Position(1,2)
        #See if i put the rook on (1,2) is occupied and will harm the bishop
        #What should not
